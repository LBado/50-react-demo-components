...
const Testimonial = () => {
  ...
  const [testimonialNo, setTestimonialNo] = useState(6);
  const [timeout, setTimeout] = useState(10000);

  useInterval(() => {
    // setTime((prev) => prev - 1);
    // const percent = (time / 500) * 100;
    // setTimeoutPercentage(percent);
    // if (percent <= 0) {
    //   setTimeout(null);
    //   setTime(500);
    // }
    // console.log(percent);
    if (testimonialNo + 1 === testimonials.length ) {
      setTestimonialNo(0);
      return;
    }
    setTestimonialNo((prev) => prev + 1);
  }, timeout);
  ...
  return (
    <div className={styles['testimonial-box']}>
      ...
      <section className={styles.testimonial}>
        <span>&#8221;</span>
        <p className={styles.text}>{testimonials[testimonialNo].text}</p>
        <span>&#8220;</span>
      </section>
      <div className={styles.profile}>
        <img alt="profile" src={`${testimonials[testimonialNo].photo}`}></img>
        <div className={styles.info}>
          <h4>{testimonials[testimonialNo].name}</h4>
          <span>{testimonials[testimonialNo].position}</span>
        </div>
      </div>
    </div>
  );
